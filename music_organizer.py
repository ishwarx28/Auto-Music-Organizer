...
def organize_music(root, filetypes, words_to_remove, top_k, preferred_spaces, min_count, dry_run=True):
    ...
    # Create output directory in the script's location
    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_dir = os.path.join(script_dir, "outputs")
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    clusters_json_path = os.path.join(output_dir, 'clusters.json')
    with open(clusters_json_path, 'w', encoding='utf-8') as writer:
        json.dump(clusters, writer, indent=4)
    print(f"Cluster information saved to '{clusters_json_path}'.")
    ...
    if not dry_run:
        ...
        for cluster_name, song_list in clusters.items():
            ...
            print(f"{copied_count} files copied to '{cluster_name}'.")
        print(f"Clusters and songs have been organized into '{organised_root}'.")
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--disable_dry_run", action='store_false', default=True, help="Run without copying files (default: True)")
    args = parser.parse_args()

    organize_music(args.root, args.filetypes, args.words_to_remove, args.top_k, args.preferred_spaces, args.min_count, dry_run=args.disable_dry_run)
